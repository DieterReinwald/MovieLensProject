...
knitr::opts_chunk$set(echo = FALSE, fig.height = 3, fig.width = 4, fig.align = "center", message = FALSE, warning = FALSE)
```
\newpage
# Executive summary
...
```{r start timer preprocess explore visualize}
#Start timer
...
```{r top 20 most rated movies, fig.width=6}
...
In terms of temporal development the histogram *Ratings per year* shows different things: - the data set contains ratings for the years `r min(edx$rating_year)` to `r max(edx$rating_year)`. - users rated movies particularly often in the years 1996, 2000, and 2005. In contrast, there were only very few ratings in the years 1995, 1998, and 2009 compared to other years.

```{r distribution of movies rated, fig.width=5}
...
```{r distribution of movies released, fig.height=3.5, fig.width=6}
...
```{r total movie ratings per genre, fig.height=3.5, fig.width=5}
...
```{r distribution of movie ratings per genre, fig.height=6, fig.width=6}
...
```{r top 10 most popular movies, fig.width=6}
...
```{r top 10 most unpopular movies, fig.width=6}
...
```{r movieId histogram, fig.height=4, fig.width=6}
# MovieId histogram
edx %>% 
   ggplot(aes(movieId)) + 
   geom_histogram(binwidth = 1, color = "black") + 
...
```{r userId histogram, fig.height=4, fig.width=6}
# UserId histogram
edx %>% 
   ggplot(aes(userId)) + 
   geom_histogram(binwidth = 1, color = "black") + 
...
```{r number of ratings per week, fig.height=4, fig.width=6}
...
```{r correlation analysis, fig.height=5, fig.width=6}
...
```{r stop timer preprocess explore visualize}
#Stop timer
...
```{r ratings over movieId, fig.height=5, fig.width=6}
...
```{r ratings over userId, fig.height=5, fig.width=6}
# Ratings over userId 
edx %>% 
   group_by(userId) %>%
   summarize(n = n(), mean = mean(rating), se= sd(rating)) %>%
...
```{r ratings over genres, fig.height=3, fig.width=6}
# Ratings over genres
edx_separated %>%
   group_by(genres) %>%
   summarize(n = n(), mean = mean(rating), se= sd(rating)) %>%
...
```{r ratings over movie_year, fig.height=3, fig.width=6}
...
```{r ratings over rating_year, fig.height=3.5, fig.width=6}
...
```{r ratings over rating_month and rating_day, fig.height=3.5, fig.width=6}
...
```{r start timer Train and compare different models}
#Start timer
...
```{r stop timer Train and compare different models}
#Stop timer
...
```{r titles of movies with no rating, fig.width=5}
# Get titles of movies with no rating
validation %>% 
   filter(is.na(final_predict)) %>% 
   select(title, rating) %>% 
   knitr::kable()
```
...
In summary, the following results of the project ca n be pointed out: The given target value of 0.86490 could be undercut in several cases with different models. The best model trained was model `r best_modelid` - `r best_method`. This model reached a RMSE value of `r final_model_rmse` in the validation.
